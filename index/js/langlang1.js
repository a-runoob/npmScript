var RongCloud={
    data:{
        ...
        msglist:[],
        hasMsg:true,
        appKey:'vnroth0kvbo8o',
        dp:[],//未读消息的display
    },
    targetUser:null,
    toChatInfo:function(user,targetUser){
        //点击消息列表  跳到不同的好友  给targetUser赋值
        RongCloud.user=user;
        RongCloud.targetUser=targetUser;
    },
    //连接服务器
    connectServer:function(){
        ...
        RongIMLib.RongIMClient.init(RongCloud.data.appKey);
        // 连接服务器 
            ...
            onReceived: function (message) {
                ...
                switch(message.messageType){        
                    ...
                    var div='<div class="receiveMsg">'+
                        '<img src="'+message.content.user.portrait+'" class="usericon"></img>'+
                        "<span class='messageinfo'>"+decodeURIComponent(message.content.content)+'</span></div>'
                    $('.innerest').append(div);
                    //滚动条 高度
    ...
    sendTextMsg:function(){
        var msg = new RongIMLib.TextMessage({ content: encodeURIComponent($('.message').val()), extra: RongCloud.targetUser ,user:RongCloud.user});
    ...
    fileUp:function(me) {
        ...
        var ratio=0.75;
        var size = me.files[0].size;
        // if(size>300000&&size<800000){
        //     var ratio=0.5;
        // }
        // if(size>=800000&&size<=1200000){
        //     var ratio=0.3;
        // }
        // if(size>=1200000&&size<1400000){
        //     var ratio=0.15;
        // }
        // if(size>1400000){
        //     var ratio=0.01;
        ...
        try {
            base64Image({
                ...
                ratio:ratio,                            /*【选填】压缩率默认0.75 */
                file: me,                              /*【必填】对应的上传元素 */
                callback:  (base64Str)=> {        /*【必填】处理成功后的回调函数 */
                    var imageUri = ''; // 上传到自己服务器的 URL。
                    var base64Str =base64Str ;
                    // window.open(base64Str)
                    var msg = new RongIMLib.ImageMessage({content: base64Str,extra: RongCloud.targetUser ,user:RongCloud.user});
                    var conversationType = RongIMLib.ConversationType.PRIVATE; // 单聊, 其他会话选择相应的消息类型即可
                    var targetId =this.targetUser.targetId ; // 目标 Id
                    RongIMClient.getInstance().sendMessage(conversationType, targetId, msg, {
                        onSuccess:  (message)=> {
            ...
            onSuccess: (list)=> {
                ...
                list.forEach((item,index)=>{
                    ...
                    RongIMLib.RongIMClient.getInstance().getUnreadCount(conversationType, targetId, {
                        onSuccess:(count)=>{
                            RongCloud.data.unreadCount[index]=count;
                            //未读消息  的 display
                            RongCloud.data.dp[index]= RongCloud.data.unreadCount[index]== 0 ? 'none' : 'inline';
                            console.log('未读消息数量：'+count)
                            console.log(RongCloud.data.dp[index])
                            ...
                            if(item.latestMessage.senderUserId==RongCloud.user.id){
                                // var targetU=item.latestMessage.content.extra;
                                var listId =item.latestMessage.content.extra.targetId
                                var name=item.latestMessage.content.extra.name;
                                var portrait =item.latestMessage.content.extra.portrait;
                            }else{
                                var listId=item.targetId;
                                var name = item.latestMessage.content.user.name;
                                var portrait =item.latestMessage.content.user.portrait;
                            }
                            console.log(chatContent);
                            div +='<div class="listItem" onclick=" window.location.replace(`chatRoom1.html?='+listId+'='+name+'='+portrait+'`);">'+
                                        '<div class="portrait">'+
                                            '<img src="'+portrait+'">'+
                                            '<span class="unreadCount" style="display:'+RongCloud.data.dp[index]+'">'+RongCloud.data.unreadCount[index]+'</span>'+
                                        '</div>'+
                                        '<div class="content">'+
                                            '<p class="name nw1">'+name+'</p>'+
    ...
    // //获取指定未读消息数量
    // getUnreadMsg:function(targetId,index){
    //     var conversationType = RongIMLib.ConversationType.PRIVATE;
